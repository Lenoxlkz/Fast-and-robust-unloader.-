...
export async function POST(req: NextRequest) {
  try {
    ...
    // 0D. Instagram (Posts, Carousels, Reels)
    const isInstagram = targetUrl.includes('instagram.com') || targetUrl.includes('instagr.am');
    if (isInstagram) {
      try {
        const isReel = targetUrl.includes('/reel/') || targetUrl.includes('/reels/') || targetUrl.includes('/tv/');
        ...
        const cleanLookupUrl = shortcode ? (isReel ? `https://www.instagram.com/reel/${shortcode}/` : `https://www.instagram.com/p/${shortcode}/`) : targetUrl;

        // 1. Fetch crawler metadata
        try {
          ...
          if (botRes.ok) {
            ...
            if (ogTitle) {
              ...
              if (!t.toLowerCase().includes('instagram') && !t.toLowerCase().includes('login')) {
                igTitle = t;
              }
            }
            const authorMatch = botHtml.match(/content=["']([^"':]+) on Instagram:/i);
            if (authorMatch) {
        ...
        seriesTitle = igTitle ? igTitle.slice(0, 100) : (author ? (isReel ? `Reel de ${author}` : `Instagram de ${author}`) : (isReel ? 'Instagram Reel' : 'Instagram Post'));

        if (isReel) {
          chapters.push({
            id: '1',
            name: author ? `${author} - Reel` : (shortcode ? `Reel ${shortcode}` : 'Video Reel'),
            url: targetUrl
          });
